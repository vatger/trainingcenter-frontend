...
export function TrainingStationListView() {
    ...
    const { data: trainingStations, loading: loadingTrainingStations, setData: setTrainingStations } = useApi<TrainingStationModel[]>({
    ...
    function syncStations() {
        setSubmitting(true);
        axiosInstance.post("/administration/training-station/sync")
    ...
    return (
        <>
            ...
            <Card>
                <Table paginate searchable defaultSortField={1} columns={TSLListTypes.getColumns(navigate)} data={trainingStations ?? []} loading={loadingTrainingStations} />
