    ...
    return [
        ...
        },
        {
            ...
            cell: row => {
                return (
                    <div className={"flex"}>
                        <Button color={COLOR_OPTS.PRIMARY} variant={"twoTone"} icon={row.read ? <TbEyeOff size={20}/> : <TbEye size={20}/>} onClick={() => toggleRead(row.id)} size={SIZE_OPTS.SM}></Button>
                        <Button color={COLOR_OPTS.DANGER} className={"ml-2"} variant={"twoTone"} icon={<TbTrash size={20}/>} size={SIZE_OPTS.SM} onClick={() => deleteNotification(row.id)}></Button>
                    </div>
                )
            }
        }
