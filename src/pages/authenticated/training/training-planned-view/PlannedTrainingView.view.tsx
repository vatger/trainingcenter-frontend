        ...
        <>
            ...
            <Card>
                ...
                <div className={"grid grid-cols-1 md:grid-cols-2 gap-5"}>
                    ...
                    <Input labelSmall preIcon={<TbId size={20} />} label={"Training Typ"} disabled value={`${trainingSession?.training_type?.name} (${StringHelper.capitalize(trainingSession?.training_type?.type)})`} />
                    <Input labelSmall preIcon={<TbRadar size={20} />} label={"Station"} disabled value={`${trainingSession?.training_station?.callsign} (${trainingSession?.training_station?.frequency?.toFixed(3)})`} />
                    <Input labelSmall preIcon={<TbCalendarEvent size={20} />} label={"Datum"} disabled value={dayjs.utc(trainingSession?.date).format(Config.DATETIME_FORMAT)} />
                </div>

                <Separator/>

                <div className={"grid grid-cols-1 md:grid-cols-2 gap-5"}>
                    <Input labelSmall preIcon={<TbListCheck size={20} />} label={"Mentor"} disabled value={`${trainingSession?.mentor?.first_name} ${trainingSession?.mentor?.last_name} (${trainingSession?.mentor?.id})`} />
                    ...
                    <RenderIf
                        ...
                        elementTrue={
                            <>
                                <Input labelSmall preIcon={<TbId size={20} />} label={"CPT Prüfer"} disabled value={`${trainingSession?.cpt_examiner?.first_name} ${trainingSession?.cpt_examiner?.last_name} (${trainingSession?.cpt_examiner?.id})`} />
                                <Input labelSmall preIcon={<TbId size={20} />} label={"ATSIM Test Bestanden"} disabled value={trainingSession?.cpt_atsim_passed ? "Ja" : "Nein"} />
                            </>
                        }
                    />
                </div>
